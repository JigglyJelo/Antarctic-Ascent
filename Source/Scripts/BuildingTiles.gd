...
func tile_controls() -> void:
	...
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
		...
		if Game.player != null and mouse_pos.distance_squared_to(Game.player.global_position) > 225 and mouse_pos.x > -180 and mouse_pos.x < 180:
			var cell: Vector2i = local_to_map(to_local(mouse_pos))
			if snow >= 0.5 and get_cell_tile_data(cell) == null and ground_layer.get_cell_tile_data(cell) == null:
				set_cells_terrain_connect([cell],0,0,true)
				set_snow(snow-0.5)
	...
	#Erase Tiles
			...
			destroy_tile(get_global_mouse_position(),true)
